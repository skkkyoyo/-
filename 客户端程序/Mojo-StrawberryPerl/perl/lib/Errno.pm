...
"MSWin32-x86-multi-thread-64int-6.3" or
	die "Errno architecture (MSWin32-x86-multi-thread-64int-6.3) does not match executable architecture ($Config{'archname'}-$Config{'osvers'})";
...
our %EXPORT_TAGS = (
    ...
    WINSOCK => [qw(
	WSAEHOSTUNREACH WSAECONNREFUSED WSAECONNRESET WSAEMFILE WSAENETUNREACH
	WSAEADDRNOTAVAIL WSAENOPROTOOPT WSAEHOSTDOWN WSAEINVALIDPROVIDER
	WSAEREFUSED WSAEINVALIDPROCTABLE WSAEPROVIDERFAILEDINIT WSAESTALE
	WSAETOOMANYREFS WSAEPROTONOSUPPORT WSAEADDRINUSE WSAESOCKTNOSUPPORT
	WSAEWOULDBLOCK WSAEBADF WSAEAFNOSUPPORT WSAEOPNOTSUPP WSAEPFNOSUPPORT
	WSAECONNABORTED WSAEALREADY WSAECANCELLED WSAEMSGSIZE WSAEINPROGRESS
	WSAETIMEDOUT WSAENAMETOOLONG WSAEINVAL WSAENOTEMPTY WSAEDISCON
	WSAEPROCLIM WSAENOTCONN WSAENOTSOCK WSAEISCONN WSAEPROTOTYPE
	WSAEREMOTE WSAENETRESET WSAEACCES WSAENOMORE WSAEINTR WSAEUSERS
	WSAEDESTADDRREQ WSAELOOP WSAENOBUFS WSAESHUTDOWN WSAENETDOWN WSAEDQUOT
	WSAEFAULT
    )],
);
